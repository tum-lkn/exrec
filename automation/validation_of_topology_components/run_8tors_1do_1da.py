...
from entities import ClockGenerator, Server, PhysicalMachine, Experiment, DemandObliviousSwitch, \
    DemandObliviousController, \
    ListDataCollector, MoongenTrafficCollector, DockerStatsCollector
...
if __name__ == "__main__":
    ...
    EXPERIMENT_NAME = f"{MILESTONE_NAME}_1do_1da"
    ...
    myclkgen = ClockGenerator(CLKGEN_PORT, 0.005, GIT_PATH, USE_PYTHON_CLK, vmax=VLAN_MAX, duty=0.9)
    ...
    myflows = traffic_inputs.get_flows_Y_udp_moongen_8tors(myservers, rate=10)
    mycachectr = da_controllers.get_staticlist_x8_1da_all_flows(myservers)
