...
if [[ "$1" =~ ^[0-9]+$ ]] && [ "$1" -ge 0 -a "$1" -le 64 ]; then
	...
	VAGRANT_TEMPLATE=$PROJECT/end-host/vagrant-ml-gpu/Vagrantfile.template
	...
	BOOT_SCRIPT=$PROJECT/end-host/vagrant-ml-gpu/vagrant-docker-vm-boot.sh
	if [ ! -f "$BOOT_SCRIPT" ]; then
		echo "The boot script does not exist, cannot create VM!"
		exit 1
	fi

	# Checking that the GPT2 data is there
	GPT2=$PROJECT/end-host/vagrant-ml-gpu/gpt-2

	cp $PROJECT/end-host/vagrant-ml-gpu/gpt2-train-horovod.py ${GPT2}/train-horovod.py
	...
	# TODO(user) replace hostnames if they differ
	case $HOSTNAME in
		server1)
